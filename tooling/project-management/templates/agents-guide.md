...
```yaml
references: blocked-by:t-3, t-1, d-2, i-1
```
...
| --------- | ------------------------------------------------- |
| `t-x`     | Task (in `tasks/` or inside an initiative folder) |
| `d-x`     | Document in `docs/[d-x]-title.md`                 |
| `i-x`     | Initiative at `initiatives/[i-x]-name/README.md`  |

**Note:** Task IDs are globally unique. A task like `t-42` can be referenced directly regardless of whether it's in `tasks/` or inside an initiative folder.

### Relationship Prefixes

- `blocked-by:t-x` — This task is blocked by another task (only valid for tasks)
...
```yaml
# Referencing a task (works for any task, regardless of location)
references: t-5

# Blocking reference
references: blocked-by:t-3, d-1

# Mixed references
references: t-1, t-7, d-2, i-3
```
...
**When a standalone task is done** (tasks in `tasks/` folder):
...
```bash
# Move completed standalone task to archive
mv tasks/[t-6]-dark-mode-toggle.md archive/tasks/
```

**When a task inside an initiative is done:**

1. Set `status: done` in the frontmatter
2. **Keep the task file in place** — do NOT move it yet
3. The initiative's progress percentage will update automatically

**IMPORTANT:** Never move individual tasks out of an initiative folder. Tasks stay with their initiative until the entire initiative is archived.

**When a document is no longer needed:**

1. Move the file to `archive/docs/`

**When an initiative is fully complete** (all tasks done):

1. Set `status: done` in the initiative's README frontmatter
2. Move the **entire initiative folder** to `archive/initiatives/`
   - This includes the README.md and all task files inside
   - Do NOT move tasks individually to `archive/tasks/`

**Example:**

```bash
# Move completed initiative (with all its tasks) to archive
mv initiatives/[i-1]-user-management archive/initiatives/
```
...
3. Reference tasks using flat IDs (e.g., `t-42`) - task IDs are globally unique
4. Run compile and verify no errors
